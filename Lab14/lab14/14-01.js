    ...
    if (req.method === 'GET' && pathname === '/') {
        ...
        switch (path) {
            ...
            case 'api/auditoriumsWithComp1':
                console.log('GET api/auditoriumsWithComp1');
                service.getHandler(req, res, database.getComputerClassesFrom1);
                break;
            case 'api/puplitsWithoutTeachers':
                console.log('GET api/puplitsWithoutTeachers');
                service.getHandler(req, res, database.getPulpitsWithoutTeachers);
                break;
            case 'api/pulpitsWithVladimir':
                console.log('GET api/pulpitsWithVladimir');
                service.getHandler(req, res, database.getPulpitsWithVladimir);
                break;
            case 'api/auditoriumsSameCount':
                console.log('GET api/auditoriumsSameCount');
                service.getHandler(req, res, database.getAuditoriumsWithSameCount);
                break;
            case 'api/getPulpitsByFaculty':
                console.log(`GET api/getPulpitsByFaculty/${codeParameter}`);
                if (codeParameter === undefined) {
                    service.errorHandler(res, 400, 'Invalid parameters');
                } else {
                    service.getHandler(req, res, database.getPulpitsByFacultyFluentAPI, codeParameter);
                }
                break;
            case 'api/transaction':
                console.log('GET api/transaction');
                service.getHandler(req, res, database.transaction);
                break;
            default:
