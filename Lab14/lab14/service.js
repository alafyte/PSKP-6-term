...
class Service {
    ...
    workWithTeacher = (req, res, databaseFunc) => {
        ...
        req.on('end', () => {
            json = JSON.parse(json);
            if (req.method === 'POST') {
                if (json.TEACHER === undefined || json.TEACHER_NAME === undefined || json.PULPIT === undefined) {
                    this.errorHandler(res, 422, "Invalid teacher parameters");
                    return;
                }
                else if (json.TEACHER.trim() === "" || json.TEACHER_NAME.trim() === "" || json.PULPIT.trim() === "") {
                    this.errorHandler(res, 422, "Invalid teacher parameters");
                    return;
                }
            }
            if (json.TEACHER === undefined || json.TEACHER.trim() === "") {
